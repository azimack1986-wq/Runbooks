...
.DESCRIPTION
    Version 1.3 - RDM NAA extraction now handles both VML and NAA DeviceName
    formats. VML format (vml.XX...) is decoded by skipping the 10-char header
    (type + LUN# + pad) and prepending naa. to the remaining hex. NAA format
    (naa.XX...) is matched directly. Get-HardDisk API fallback retained.
    Per-host ping filter + real-time progress retained from v1.1.
...
.EXAMPLE
    .\Get-RunbookVMInventory_v1.3.ps1 -OutputPath "\\server\share\VMInventory"
#>
