...
.DESCRIPTION
    Version 1.8 - PrimaryLocation now derived from the first 3 characters of
    the ESX cluster name (was incorrectly slicing the VM hostname). Datastore
    loop no longer breaks early on RDM pointer datastores -- SC is now resolved
    from the first datastore whose name contains the SC pattern, independently
    of which datastore is selected as VMwareDatastore.
...
# ── Helper: Derive Primary Location from ESX cluster name (first 3 chars) ────
function Get-PrimaryLocation {
    param ([string]$ClusterName)
    if (-not $ClusterName) { return $null }
    if ($ClusterName.Length -ge 3) { return $ClusterName.Substring(0, 3) }
    return $ClusterName
}
...
function Get-VMInventoryRecord {
    ...
    foreach ($dsMoRef in $VM.ExtensionData.Datastore) {
        ...
        if (-not $dsName -or $dsName -match 'vswap') { continue }
        # VMwareDatastore/DatastoreCluster: first non-vswap, non-RDM datastore.
        if (-not $datastoreName -and $dsName -notmatch '(?i)rdm') {
            $datastoreName    = $dsName
            $datastoreCluster = $Lookups.DSStoragePodMap[$dsMoRef.ToString()]
        }
        # SC: scan all datastores until the pattern is found (RDM stores lack it).
        if (-not $sc) { $sc = Get-SCFromDatastore -DatastoreName $dsName }
        if ($datastoreName -and $sc) { break }
    }
    ...
    return [ordered]@{
        ...
        SC               = $sc
        PrimaryLocation  = Get-PrimaryLocation -ClusterName $cluster
